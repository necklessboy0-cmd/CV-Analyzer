...
def build_pdf(cv_text: str) -> bytes:
    ...
    story.append(Paragraph(first, title))
    ...
    for line in lines[1:]:
        if not line: continue
        clean = re.sub(r"^[•●▪◦\-*]+\s*", "", line).strip()
        if clean.lower() in heading_names: story.append(Paragraph(clean.upper(), heading))
        elif re.match(r"^[•●▪◦\-*]\s*", line): story.append(Paragraph("• " + clean, bullet))
        else: story.append(Paragraph(clean.replace("&","&amp;"), body))
...
def clear_for_new_upload(filename: str) -> None:
    if st.session_state.get("cv_filename") != filename:
        for key in ("cv_result","cv_checks","cv_filename","cv_structure","enhanced_cv","enhanced_result","verification","output_files","enhancement_complete"):
...
def handle_error(exc: Exception) -> None:
    ...
    if "429" in message or "rate" in message or "quota" in message: st.error("Gemini rate limit or quota reached. Please try again later.")
    elif "401" in message or "403" in message or "api key" in message: st.error("Gemini authentication failed. Check GEMINI_API_KEY in Streamlit Secrets.")
    elif "json" in message: st.error("Gemini returned an unreadable JSON format. Please try again.")
    elif "pdf" in message or "document" in message or "decode" in message: st.error("The document could not be read. Try exporting it again as PDF, DOCX, or TXT.")
    else: st.error("The CV could not be processed. Please verify the file and try again.")
...
def main() -> None:
    ...
    clear_for_new_upload(uploaded.name)
    ...
    if st.button("Analyze CV", type="primary", use_container_width=True):
        try:
            ...
            set_workflow("Content Extraction","done","Content extracted with page boundaries retained.")
    ...
    if "cv_result" in st.session_state and "cv_checks" in st.session_state:
        ...
        if not st.session_state.get("enhancement_complete"):
            ...
            st.markdown('<div class="panel"><h3>Enhance My CV</h3><p>Generate a professionally improved CV using the original CV as the factual source of truth. The app will validate and re-analyze the enhanced version before calling it ready.</p></div>', unsafe_allow_html=True)
            if st.button("✨ Enhance My CV", type="primary", use_container_width=True):
                try:
                    set_workflow("CV Enhancement","active","Gemini is creating a fact-preserving professional version of your CV.")
                    enhanced = enhance_cv_with_gemini(st.session_state.cv_text, st.session_state.cv_result, api_key)
                    ...
                    verification = verify_enhancement(st.session_state.cv_text, enhanced_text, st.session_state.cv_result, enhanced_result, api_key)
